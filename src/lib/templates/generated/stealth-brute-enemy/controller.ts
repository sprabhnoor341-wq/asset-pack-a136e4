class StealthBruteEnemy extends pc.ScriptType {
  static scriptName = 'stealth-brute-enemy';

  @attribute({ type: 'number', default: 4, min: 2, max: 10 })
  visionRange: number = 4;

  @attribute({ type: 'number', default: 40, min: 20, max: 90 })
  visionConeAngle: number = 40;

  @attribute({ type: 'number', default: 1.5, min: 0.5, max: 5 })
  patrolSpeed: number = 1.5;

  @attribute({ type: 'number', default: 2.5, min: 1, max: 8 })
  chaseSpeed: number = 2.5;

  @attribute({ type: 'number', default: 1.2, min: 0.1, max: 5 })
  attackDamage: number = 1.2;

  @attribute({ type: 'number', default: 1.5, min: 0.5, max: 3 })
  attackRange: number = 1.5;

  @attribute({ type: 'number', default: 6, min: 2, max: 15 })
  hearingRange: number = 6;
  ...
  @attribute({ type: 'number', default: 25, min: 5, max: 60 })
  rotateSpeed: number = 25;
